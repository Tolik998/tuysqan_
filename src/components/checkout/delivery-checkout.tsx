...
import { deliveryOrderSchema, type DeliveryOrderInput } from "@/lib/validation";
...
export function DeliveryCheckout() {
  ...
  const form = useForm<FormValues>({
    resolver: zodResolver(
      deliveryOrderSchema.omit({ items: true, total: true }),
    ),
    defaultValues: {
      customerName: "",
      deliveryAddress: "",
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_420px]">
      <section>
        ...
        <h1 className="text-3xl font-bold sm:text-4xl">Доставка</h1>
        <p className="mt-3 text-sm leading-6 text-[#020D13]/55">
          Заказ сохраняется в системе, затем откроется WhatsApp с готовым
          сообщением для Tuysqan.
        </p>
        <form
